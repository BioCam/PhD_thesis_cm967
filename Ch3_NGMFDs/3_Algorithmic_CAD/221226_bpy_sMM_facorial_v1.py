...
# - - - - - - - - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - 
tr_factorial_df = pd.read_csv('/Users/camillomoschner/Desktop/PhD_thesis/Ch3_NGMFDs/2_2PP-PL_Hybrid/v5_procedural_feature_generation/smm_runs.csv')    
...
for repeats in range(0,1):
    ...
    for row in range(len(tr_factorial_df)):
        ...
        if row%1==0:
            ...
            location=(new_x_pos - current_tr_info['tr_w']/2, 
            current_tr_info['tr_l']/2+4, 0), scale=(1, 1, 1))
        ...
        for replicates in range(0,3):
            simple_mm(current_tr_info['tr_w'], current_tr_info['tr_l'], current_tr_info['tr_h'])
