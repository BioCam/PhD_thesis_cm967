...
# - - - - - - - - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - 
st_factorial_df = pd.read_csv('/Users/camillomoschner/Desktop/PhD_thesis/Ch3_NGMFDs/2_2PP-PL_Hybrid/v5_procedural_feature_generation/stmm_runs.csv')
## Set the dimensions of the objects
save_file = False
file_name = "221229_bpy_stmm_w_indices.stl"
#tr_width = 1.6
#tr_height = 1.6
#tr_length = 80
...
for repeats in range(0,1):
    ...
    for row in range(len(st_factorial_df.iloc[:100,:])):
        current_tr_info = st_factorial_df.iloc[row,:]
        # add text as indices & fiduciaries
        if row%2==0:
            bpy.ops.object.text_add(enter_editmode=False, align='WORLD', 
            location=(new_x_pos , 
            current_tr_info['st_l']/2+4, 0), scale=(1, 1, 1))
        ...
        for replicates in range(0,1):
            st_mm(current_tr_info['tr_w'], current_tr_info['tr_l'], current_tr_info['tr_h'],
                  current_tr_info['st_w'], current_tr_info['st_l'], current_tr_info['st_h'])
            bpy.context.object.location.x = new_x_pos + current_tr_info['tr_w']/2 + current_tr_info['st_w']#(tr_width+spacing) * i
            # analyse latest generated object to adjust new location for next object
            active_object = bpy.context.view_layer.objects.active
            active_object.location[2] = current_tr_info['st_h']/2
            ...
            new_x_pos = active_object.location[0] + spacing + current_tr_info['tr_w']/2 + current_tr_info['st_w']
# - - - - - - - - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - 
